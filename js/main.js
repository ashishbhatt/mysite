;(function () {
	...
	var burgerMenu = function() {
		...
		$('body').on('click', '.js-ashish-nav-toggle', function(event){
	...
	var clickMenu = function() {
		...
		$('a:not([class="external"])').click(function(event){
		    ...
		    if ( navbar.is(':visible')) {
		    	...
		    	$('.js-ashish-nav-toggle').removeClass('active');
		    }
	...
	var windowScroll = function() {
		...
		$(window).scroll(function(event){
		   	...
		   	var header = $('#ashish-header'),
				scrlTop = $(this).scrollTop();

			if ( scrlTop > 500 && scrlTop <= 2000 ) {
				header.addClass('navbar-fixed-top ashish-animated slideInDown');
			} else if ( scrlTop <= 500) {
				if ( header.hasClass('navbar-fixed-top') ) {
					header.addClass('navbar-fixed-top ashish-animated slideOutUp');
					setTimeout(function(){
						header.removeClass('navbar-fixed-top ashish-animated slideInDown slideOutUp');
					}, 100 );
				}
			} 
	...
	var servicesAnimate = function() {
		...
		if ( $('#ashish-our-services').length > 0 ) {	
			$('#ashish-our-services .to-animate').each(function( k ) {
	...
	var servicesWayPoint = function() {
		...
		if ( $('#ashish-our-services').length > 0 ) {
			$('#ashish-our-services').waypoint( function( direction ) {
	...
	var featuresAnimate = function() {
		...
		if ( $('#ashish-features').length > 0 ) {	
			$('#ashish-features .to-animate').each(function( k ) {
	...
	var featuresWayPoint = function() {
		...
		if ( $('#ashish-features').length > 0 ) {
			$('#ashish-features').waypoint( function( direction ) {
	...
	var testimonialsAnimate = function() {
		...
		if ( $('#ashish-testimonials').length > 0 ) {	
			$('#ashish-testimonials .to-animate').each(function( k ) {
	...
	var testimonialsWayPoint = function() {
		...
		if ( $('#ashish-testimonials').length > 0 ) {
			$('#ashish-testimonials').waypoint( function( direction ) {
	...
	var pricingAnimate = function() {
		...
		if ( $('#ashish-pricing').length > 0 ) {	
			$('#ashish-pricing .to-animate').each(function( k ) {
	...
	var pricingWayPoint = function() {
		...
		if ( $('#ashish-pricing').length > 0 ) {
			$('#ashish-pricing').waypoint( function( direction ) {
	...
	var pressAnimate = function() {
		...
		if ( $('#ashish-press').length > 0 ) {	
			$('#ashish-press .to-animate').each(function( k ) {
	...
	var pressWayPoint = function() {
		...
		if ( $('#ashish-press').length > 0 ) {
			$('#ashish-press').waypoint( function( direction ) {
